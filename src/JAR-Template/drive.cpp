...
// Setting PID/Swing conditions for customizability for each movement
...
// get absolute heading utilizes the reduce_0_to_360 method to get the angle in that range, and uses the gyro scale variable to account for constant gyro drift. You set the gyro scale variable to what the gyro is constantly reading as 360 degrees and that value will equal the 360 degrees - this functions basically just gets the heading from the gyro.
float Drive::get_absolute_heading()
{
  return (reduce_0_to_360(Gyro.rotation() * 360.0 / gyro_scale));
}

// get left and right position in inches of the drive
...
// TURN TO ANGLE FUNCTIONS + OVERLOADING FUNCTIONS
...
void Drive::turn_to_angle(float angle, float turn_max_voltage, float turn_settle_error, float turn_settle_time, float turn_timeout, float turn_kp, float turn_ki, float turn_kd, float turn_starti)
{
  ...
  // creates the pid while loop, using the is settled function as the condition for ending the turn pid or not
  while (turnPID.is_settled() == false)
  {
    ...
    float output = turnPID.compute(error);                       // Compute the PID output
    output = clamp(output, -turn_max_voltage, turn_max_voltage); // clamps output to turn max and turn min voltage
    drive_with_voltage(output, -output);                         // drives the actual motors with the output
    task::sleep(10);                                             // sleep for 10 ms
  }
  // stop the motor and set to hold
  DriveL.stop(hold);
  DriveR.stop(hold);
}

// DRIVE DISTANCE FUNCTIONS + OVERLOADING FUNCTIONS
...
// overloading functions - RIGHT SWING TO ANGLE
...
// public api getter functions for odom positions
...
{
  // first parameter is the distance from the current position to the desired position (Finds the hypotenuse of the c value given the x and y differences being a and b)
